...
class Combo():
    def __init__(self, name = "", hand_req = {}, hand_or_deck = {}, deck_req = {}, extra_req = {}, subcombos = [], folder = ""):
        self.name  = name
        self.hand  = hand_req
    ...
    def isCombo(self, hand, deck, extra):
        return self.allThere(self.hand, hand) and self.allThere(self.deck, deck) and self.allThere(self.extra, extra)

    def allThere(combo_req, combo_ava):
        for element in combo_req:
            if (element in combo_ava):
                combo_ava.remove(element)
            else:
                return False
...
#### SAVE LOAD AND EDITOR FUNCTIONS ####
    ...
    def load(self, name, folder):
        ...
        self.subcombos = common.string2List(deck_raw[10])
        self.folder = deck_raw[12]
